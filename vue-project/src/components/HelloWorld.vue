...
import { spintaxListStore } from '@/store/spintaxStore.ts';
import { ref, computed } from 'vue';
import axios from 'axios';
import { Constants } from '@/constants.ts';

const store = spintaxListStore();
const spintaxInput = ref<string>("");
const resultsCount = ref<number>(Constants.DEFAULT_RESULTS_COUNT);
const results = ref<string[]>([]);
const errorMessage = ref<string>("");
const noData = ref<boolean>(false);
const lessData = ref<boolean>(false);
const duplicate = ref<boolean>(false);
  ...
  const checkForDuplicateOptions = (input: string): boolean => {
  ...
  const updateSliderValue = () => {
    console.log('Current results count: ts', resultsCount.value);
  };
</script>
